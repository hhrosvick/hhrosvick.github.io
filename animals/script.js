$(function(){
  $('#randomize').click(function(){
    ...
    for (var i=0; i < raw_meat.length; i++) {
      if (/\S/.test(raw_meat[i])) {tested_meat.push($.trim(raw_meat[i]));}}
    var trimmed_meat = [];
    for (var i=0; i < tested_meat.length; i++) {
      var meat_parts = tested_meat[i].split(',');
      if(meat_parts.length >= 3){
        trimmed_meat.push({name: $.trim(meat_parts[0]),
    ...
    while(right_animal_choice == left_animal_choice){right_animal_choice = Math.floor(Math.random() * (trimmed_meat.length));}
